 ...
 * </ul>
 *
 * Without this wiring, {@code rabbitTemplate.send(...)} returns when bytes
 * hit the AMQP client buffer — NOT when the broker accepts/persists/routes
 * the message — so outbox rows get stamped PUBLISHED for messages the broker
 * silently discarded. Mirrors Wallet's coordinator for parity.
 */
@Component
