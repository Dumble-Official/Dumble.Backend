...
public class CommentReactedConsumer : IConsumer<CommentReactedEvent>
{
    ...
    public async Task Consume(ConsumeContext<CommentReactedEvent> context)
    {
        var evt = context.Message;
        if (evt.CommentAuthorId == evt.ReactorId) return; // Don't notify self

        var notification = new Notification
        {
            RecipientId = evt.CommentAuthorId,
            Type = "CommentReaction",
            Title = "New Reaction",
            Body = $"{evt.ReactorName} reacted {evt.ReactionType} to your comment",
