...
public class IdempotencyKey {
    ...
    /**
     * SHA-256 hex of the request body (strict variant of Decision 3.2). When
     * present, a replay with the same key but a DIFFERENT body is rejected
     * with 409 instead of silently returning the cached response. Existing
     * rows from before V4 stay {@code null} and are treated as "no recorded
     * hash → skip comparison" so the migration is backward-safe.
     */
